# 👋 Salut, moi c’est Lucas

🎯 Développeur web en formation (Wild Code School)  
💻 Je crée des projets tech, SaaS et automatisation  
📍 France  

---

## 🚀 À propos de moi

Je suis un développeur web, focus sur :

- ⚛️ React / Next.js
- 🎨 HTML / CSS / Tailwind
- 🟨 JavaScript / TypeScript
- 🖥️ Node.js / Express
- 🐍 Python (scripts, automatisation, data)
- ⚡ FastAPI (création d’API performantes)
- 🔗 Conception et intégration d’API (REST)


Je travaille aussi sur des projets plus avancés comme :
- 🤖 automatisation (bots, trading, IA)
- 📊 SaaS (ex : outils d’analyse)

---

## 🧠 Projets en cours

🔹 **RushPlay (SaaS)**  
> Radar d’opportunités de paris sportifs basé sur des modèles de données  
Tech : Python, ML, Next.js  

🔹 **Bot Trading XAUUSD**  
> Stratégies trend following automatisées sur MT5  

---

## 🛠️ Stack

![JavaScript](https://img.shields.io/badge/JS-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)
![Tailwind](https://img.shields.io/badge/Tailwind-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)

---

## 📫 Me contacter

📧 Email : lucasguilhot7@gmail.com  
💼 LinkedIn : www.linkedin.com/in/guilhot-lucas



---

⭐ N’hésite pas à explorer mes projets !
